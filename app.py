...
def bootstrap_payload(telegram_id: int) -> dict:
    ...
    return {
        ...
        "payments": {
            "asset": CRYPTO_PAY_ASSET,
            "min_deposit_amount": format_amount(MIN_DEPOSIT_AMOUNT),
            "min_withdraw_amount": format_amount(MIN_WITHDRAW_AMOUNT),
            "max_withdraw_amount": format_amount(MAX_WITHDRAW_AMOUNT),
            "auto_payouts": get_auto_payouts(),
...
def landing():
    process_pending_invoices()
    return jsonify({"ok": True, "message": LANDING_MESSAGE, "mini_app": WEBAPP_URL, "auto_payouts": get_auto_payouts()})
...
def api_deposit():
    payload = request.get_json(silent=True) or {}
    try:
        amount = parse_amount(str(payload.get("amount", "")), minimum=MIN_DEPOSIT_AMOUNT)
